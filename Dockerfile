...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    -run 'Test(Hardening|Optimization|Repair|Points|TaskIntent|TaskModel|ModelCheckpoint|ModelStepBudget|PrepareInstanceTaskEndpoint|Doubao|AgentInstance.*Participation|GroupParticipation|ProviderResponse|RuntimeNonDirectKeyword|RuntimeCoalescesBurst|RuntimeKeepsQuietWhenProactive|RuntimeDirectName|VisualPlan|VisualStyle|VisualContinuity|VisualLifestyle|VideoLifestyle|MediaQuality|Dialogue|Memory|NativePrepareSocial|NativePrepareLifestyle|SQLiteQueryCancellation|RuntimeClose|RuntimeFixture)' .
