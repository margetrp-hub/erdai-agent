...
## 2026-08-28 插件真实健康与一键巡检

- Schema v76 将视频生成拆为独立内置插件，图片和视频分别读取真实任务、连续失败和成品附件证据；端点可达不再等同于能力可用。
- 管理 API 新增 `GET /api/v1/plugins/readiness`，汇总已启用插件并返回明确阻断项；停用和仅登记的外部能力不阻断。
- 插件中心新增“检查全部”、待实测状态和媒体真实任务计数。外部 manifest 仍不执行代码，运行权限继续由受信任适配器固定白名单控制。
- 单包部署默认进入四主题现代控制台；兼容旧页改为仅在 `ERDAI_WEBUI_MODE=legacy` 时启用，避免新安装误入陈旧 UI。
