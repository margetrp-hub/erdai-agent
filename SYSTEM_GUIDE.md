...
v0.15.2 认知增强：在现有最近事件窗口内，问题按角色、成员、线程和平台回复关系归属；另一个人的插话不替当前成员作答，已回答的问题不再当作待答。终态 ACK 保留实际文本消息 ID 与回复对象，缺少平台 ID 不伪造。该机制不解析任意长段中的多对象、多问题，也不突破原有续聊时间窗。
...
v0.15.2 交流增强：显式“先听我说”“给点建议”“一起讨论”和简答要求分别处理；短期继承限同成员、角色与线程，明确新要求或换话题可解除旧限制。情绪提示考虑否定和转述，未知或冲突不强贴标签；这是有限规则，不是心理诊断。交流偏好只影响表达，不提高工具权限。
...
记忆不是必须等用户说“记住”。当前自动采集使用显式中文规则识别称呼、偏好和习惯；并非任意自然语言事实抽取，临时项目不进入长期事实库。
...
v0.15.2 记忆纠正：同作用域内较新的明确称呼和同对象喜好替代旧自动记录，保留其它偏好与手工来源；延迟旧事件不复活旧偏好。同条消息内明确更正以最后陈述为准。旧记录会被标为过期，并受现有清理策略管理，不保证永久保留。任意身份与习惯矛盾不自动合并。

### 6.3 可选语义记忆召回（v0.15.2，默认关闭）

`memory_policy.semanticRecallEnabled` 默认 `false`，本轮没有启用生产配置或新增界面开关。管理员启用前须确认数据处理与费用，并保留其它策略字段；同时需要记忆总开关、知识检索总开关、非 `keyword` 模式、`vectorAlgorithm=remote_embedding` 和已配置可用的 `embeddingEndpointId`。

复用该嵌入端点，每个作用域单次召回最多 32 条候选、每项 1000 字符、一次批请求和 1.5 秒内部时限。向量缓存加密，热缓存只重发查询；用户与群记忆分别受限。故障回退关键词，敏感词规则命中不外发；有限敏感词规则不是完整隐私检测。删除或修改中的旧记录不能通过在途召回复活。忘记工具仍走原关键词范围，不用相似度授权删除。

本地模拟服务测试证明调用和数据边界，不证明真实嵌入模型的召回质量；候选窗口和相似度阈值仍需真实脱敏语料评价。
...
- 重启后可以恢复 queued/running 任务，复用已保存的规划和成功工具回执；供应商受理与本地落库之间仍存在不确定窗口，不保证所有调用恰好一次。
- 后台可以查看 Run、步骤、产物、错误和恢复操作。

普通聊天保持一次模型调用；只有工具、搜索、文档、图片和视频任务才进入更长的执行链。

模型规划使用带版本的加密检查点，按同一 Run 的模型步骤序号恢复原 completion 和工具调用 ID，再由既有工具回执重建后续历史；成功规划不再次请求供应商或重复累计模型用量。保存失败会在执行工具和发送进度之前停止；空回复按失败处理，允许后续合法重试。已成功的模型或工具记录损坏时禁止当作缓存缺失重新执行。请求快照用于审计，后续尚未完成的模型步骤使用当前上下文，不是逐字回放旧请求。

恢复校验当前任务修订、下发工具策略、作用域、关键规则、角色及外观绑定配置。时段、情绪与近期聊天变化不单独作废计划；关键配置不兼容、原模型端点已移除或旧版检查点没有版本信息时停止，须确认新的任务要求。同一运行实例内禁止并发重入同一 Run 的模型循环。此处不等于每步重新确认管理员身份，也不提供跨配置库、运行库与外部服务的原子授权。

发布前应处理旧版未完成任务；不自动把旧模型审计记录升级成可恢复计划。最终文字润色的独立模型调用、通用外部写入的结果核对、持久追问/审批和离线回放尚未纳入这批检查点能力。旧版程序回退也须避免直接重跑新版本的未完成任务，不通过覆盖业务数据库实施回退。
...
### 12.3 生活随拍默认策略（v0.15.3）

- 角色自拍和自拍视频的实际生成计划采用普通生活片段，未指定时选择桌边休息、沙发、出门前、阳台、楼下散步或翻书等场景；活动、动作、光线与场景一起分配。默认近景或半身，不为展示穿搭强拉全身，不固定杯子、衣服或背景。自定义机位列表保留。
- 视频默认同场景、同衣着、单镜头，一个主要小动作搭配眨眼、呼吸和停顿；不默认表演、慢动作或转场。明确的全身、舞蹈、写真、场景、衣着长度及颜色要求优先。比例与分辨率只从冻结的本次请求读取，不让外观描述或默认文案覆盖；否定比例不会成为首选。
- 用户自述的地点和活动不作为角色所在地；显式坐、站、躺、行走也不叠加冲突的默认日常活动。主语与约束归属是有限文本规则，不承诺解析任意复杂叙述。截图反馈只是审美参考，不会自动替换已选择的外观库或身份参考。
- 明确“同一场景”“接着拍”“刚才那张”“同一套衣服”时，只有同平台、实例、命名空间、会话、成员、角色与线程内，30 分钟内已终态交付、最终选中附件可对应的计划才能延续；指定引用只找该条消息，换库、过期、失败或缺少具体变量不继承。当前变更优先，传输重试复用冻结计划。此处延续的是已交付媒体的文字计划，不是上一张图片的首帧输入，也不保证像素级服装或背景一致。
- 配文自然接话，但不虚构真实拍摄、真实在场或投递成功；“在干嘛”“想看看你”不因此自动获得付费生成权限。普通非角色图片和既有工具权限不变。

以上规则是生成策略，不是成片质量保证。真实供应商成图、视频动作自然度和 QQ 最终体验需单独生成、观看并验收，不能由提示词或单元测试推定通过。
