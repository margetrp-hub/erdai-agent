...
> 文档版本：0.9.4（Schema v59）
>
> 线上事实基线：`erdai-agent:0.9.4-runtime-instances-r2`，OVH 二呆主节点 `40.160.227.173`（Tailscale `100.116.21.100`），2026-08-11 复核；已完成 `off -> shadow -> active`、真实 QQ @ 入站、`doubao-qq` 实例路由、Outbox 投递和 ACK 验收。旧 250 `47.253.214.250` 只保留轻量边缘与受保护服务，不再承载二呆完整计算栈。
>
...
输出只保留分组、近几次状态和倍率，不显示账号、Key 或内部敏感字段。分组健康应使用时间窗口评估，而不是单次探测；结尾提示“倍率越低越便宜”。雷达数据使用配置的 CodexRadar 地址或其他管理员指定事实源，并可选择读数或截图。
