...
部署脚本先停用频道接入并停止 Core 写入，再执行 SQLite 一致性备份；备份完整性校验通过后才交换应用目录。切换频道模式只更新 mode，不覆盖捕获和轮询设置。失败时恢复应用、环境、数据库和旧镜像；停止、容器改名及应用目录交换的中间失败有独立回归测试。成功后保留回滚目录及旧镜像，不自动删除回滚材料；Compose 可能重建旧服务容器，因此恢复依据是备份、旧镜像与 Compose，不能只依赖容器名称。发布会短暂中断服务，不承诺零停机。
