...
`/渠道` 登录专用的零余额普通用户，直接截取 Sub2API 渠道监控页面固定 `90m` 视图并发送图片。截图固定中文、自动关闭公告但不标记已读；页面原始卡片负责展示近 90 分钟分组状态、可用率、倍率和 5 分钟时间桶，Core 不再自行推断红黄绿。截图浏览器只接入内部 Docker 网络，不发布端口；账号密码只保存在受管凭据文件中。单组查询继续返回对应分组详情，雷达数据继续使用配置的 CodexRadar 地址或其他管理员指定事实源。
