...
func visualLifestyleInstruction(kind string) string {
	...
	if kind == "video" {
		return common + "像普通手机记录同一次生活瞬间：同一场景、同一套衣服、单镜头，一个主要小动作配自然眨眼或呼吸，保留真实短暂停顿。默认近景或半身，不安排表演、转场、慢动作或换装；用户明确要求舞蹈或全身时照办。自然肤质，脸清楚，不固定杯子或背景。"
	}
	return common + "像普通手机前置随手拍，默认近景或半身；轻微不完美的角度和构图，脸仍清晰、自然肤质，不过度磨皮。舒适的日常衣物可有自然使用痕迹，不为展示穿搭强拉全身，不固定背景、杯子或摆拍道具。"
}
