...
func visualLifestyleInstruction(kind string) string {
	...
	return common + "像真实手机原生相机随手记录的生活照，静态照片默认竖拍3:4；按选定构图和本次拍法取景，手机成像质感不等于人物举手机或照镜。全身照使用合理拍摄距离，头脚完整入镜。角度和姿势自然，脸仍清晰、自然肤质，不过度磨皮。衣物保留真实材质和自然褶皱，不固定背景、杯子或摆拍道具，不生成海报、壁纸或商业宣传图。"
}
